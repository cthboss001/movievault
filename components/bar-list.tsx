    ...
    <section className="rounded-xl border border-ink/8 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 space-y-3.5">
        {items.length > 0 ? (
          items.map((item) => (
            <div key={item.label}>
              <div className="mb-1.5 flex items-center justify-between gap-3 text-xs font-bold text-ink/60">
                <span className="truncate">{item.label}</span>
                <span className="shrink-0 tabular-nums">{item.value}</span>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-ink/8">
                <div
                  className="h-full rounded-full bg-vault transition-all duration-500"
